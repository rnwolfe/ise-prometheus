    ...
    .then((results) => {
      ...
      if (systemSummary !== "not found") {
        ...
        systemSummary["60MIN"].forEach((system) => {
          ...
          const nodeType = system.nodeType
            .replace("PAP", "PAN")
            .replace("PDP", "PSN");
      ...
      gauges.totalEndpoints.set(
        { type: "rejected" },
        parseInt(rejectedEndpoints)
      );
      gauges.totalEndpoints.set(
        { type: "anomalous" },
        parseInt(anomalousEndpoints)
      );
      ...
      gauges.misconfiguredCount.set(
        { type: "misconfigured_nas" },
        parseInt(radiusLiveLogCounters.misConfiguredNasCount)
      );
      gauges.misconfiguredCount.set(
        { type: "misconfigured_supplicant" },
        parseInt(radiusLiveLogCounters.misConfiguredSuppCount)
      );
      gauges.misconfiguredPercent.set(
        { type: "misconfigured_nas" },
        parseFloat(radiusLiveLogCounters.percentMisConfiguredNasCount)
      );
      gauges.misconfiguredPercent.set(
        { type: "misconfigured_supplicant" },
        parseFloat(radiusLiveLogCounters.percentMisConfiguredSuppCount)
      );
      gauges.radiusDropCount.set(
        parseInt(radiusLiveLogCounters.radiusDropCount)
      );
      gauges.radiusDropPercent.set(
        parseFloat(radiusLiveLogCounters.percentRadiusDropCount)
      );
      gauges.radiusRepeatCount.set(parseInt(radiusLiveLogCounters.retryCount));
      gauges.radiusRepeatPercent.set(
        parseFloat(radiusLiveLogCounters.percentRetryCount)
      );
      gauges.eapTimeoutCount.set(
        parseInt(radiusLiveLogCounters.eapTimeoutCount)
      );
      gauges.eapTimeoutPercent.set(
        parseFloat(radiusLiveLogCounters.percentEapTimeoutCount)
      );
      gauges.reauthCount.set(parseInt(radiusLiveLogCounters.totalRAuthCount));
      gauges.reauthPercent.set(
        parseFloat(radiusLiveLogCounters.percentTotalRAuthCount)
      );

      // THREAT METRICS
      gauges.topCompromisedEndpoints.set(
        { status: "connected" },
        parseInt(topCompromisedEndpoints.connected)
      );
      gauges.topCompromisedEndpoints.set(
        { status: "disconnected" },
        parseInt(topCompromisedEndpoints.disconnected)
      );
      gauges.topVulnerableEndpoints.set(
        { status: "connected" },
        parseInt(totalVulnerableEndpoints.connected)
      );
      gauges.topVulnerableEndpoints.set(
        { status: "disconnected" },
        parseInt(totalVulnerableEndpoints.disconnected)
      );
      ...
      Object.entries(topThreats.dataset).forEach((threat) => {
        ...
        gauges.topThreatsByThreat.set(
          {
            threat_name: threatName,
            severity: threatData.severity,
            status: "connected",
          },
          parseInt(threatData.endpoints.connected)
        );
        gauges.topThreatsByThreat.set(
          {
            threat_name: threatName,
          ...
          parseInt(threatData.endpoints.disconnected)
        );
        gauges.topThreatsByThreat.set(
          {
            threat_name: threatName,
          ...
          parseInt(
            threatData.endpoints.disconnected + threatData.endpoints.connected
          )
        );
      });
      ...
      threatsBySeverity.forEach((threat) => {
        gauges.topThreatsBySeverity.set(
          { severity: threat.severity, status: "connected" },
          threat.metrics.connected
        );
        gauges.topThreatsBySeverity.set(
          { severity: threat.severity, status: "disconnected" },
          threat.metrics.disconnected
        );
        gauges.topThreatsBySeverity.set(
          { severity: threat.severity, status: "all" },
          threat.metrics.connected + threat.metrics.disconnected
        );
      });
